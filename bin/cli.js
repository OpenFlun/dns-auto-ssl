...
program
    .name('dns-auto-ssl')
    .description('自动通过 DNS API 申请 SSL 证书并配置自动续期')
    .option('-e, --email <email>', '邮箱地址 (必填)')
    .option('-d, --domains <domains>', '域名，多个用逗号分隔 (必填)')
    .option('-p, --provider <provider>', 'DNS 服务商 (如 alidns, dnspod, cloudflare) (必填)')
    .option('--api-key <key>', 'API Key ID (如果服务商需要)')
    .option('--api-secret <secret>', 'API Secret (如果服务商需要)')
    .option('--api-token <token>', 'API Token (Cloudflare 等)')
