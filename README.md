...
const certDir = path.join(os.homedir(), '.dns-auto-ssl', 'certs', 'lego', 'certificates');
const domain = 'example.com';

const options = {
  key: fs.readFileSync(path.join(certDir, `${domain}.key`)),
  cert: fs.readFileSync(path.join(certDir, `${domain}.crt`))
};
